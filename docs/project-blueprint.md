...
**「今天最近的课程」列表**，两个独立入口（#30）：

| 入口 | 默认大小 | 内容 |
|---|---|---|
| 课表 · 紧凑 | **4 宽 × 2 高横条** | 最近 **2** 节**并排**两列，无标题行 |
| 课表 · 详细 | 4×4 | 最近 **4** 节竖排 + 「日期 · 第 N 周」标题行 |

- 列表 = **今天**尚未结束的课程（含正在上），按开始时间排序；**不跨天补足**
- 每条：左侧课程色竖条（与周视图同色源）+ 课名 + 「起止时间 · 节次号」+ 地点
- 正在上的课在课名旁标「正在上」
- 今天没课 / 全上完 → 空态「今天没有更多课」
- 拉伸规则：条数上限由入口决定（2 / 4），拉伸只改行高不增减条数
- 点击任意区域 → 打开 App 周视图
- 刷新（#31）：写库/改课主动刷新 + **课程边界精确刷新**（下一节课开始 / 进行中课程结束
  时刻的 OneTime 任务，自动续链）+ 15 分钟周期兜底 + 跨天/时区/重启广播

> **实现约束**：核心选择与边界计算在 `core:common/LessonWindow`（纯函数、可单测）；
> 两个入口共用一份 `WidgetData`、各自裁条数。周视图的「已上完变灰」**不适用**于小组件
> —— 列表只显示未结束的课。
