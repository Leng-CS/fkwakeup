...
| **P3** | 桌面看课表 | 桌面添加小组件 → 实例化配置显示范围与样式 → 滚动查看课程 → 点击跳转周视图 |
...
| **P0** | FR-05 桌面小组件（Glance） | 单入口、4×1 / 4×2 / 4×4 可滚动课程列表 |
...
1. **周视图顶栏的「导入」按钮** —— 常驻。已有一份课表时这是应用内**唯一**的导入入口，
   所以它**不能只在空课表时出现**（#25）。导入是这个 App 的核心动作，不该藏在空态里。
2. 空课表时，页面中央的「导入课表」主按钮（此时顶栏**不**再显示导入按钮，避免同屏两个入口）。
3. 从其它 App 分享文本进来（`ShareImportActivity`）。

> **注意**：「再导入」会**新建一个学期并切换过去**，旧学期的课程完整保留（多学期共存是既定设计）。
> 用户点完导入若觉得"课表怎么空了"，实际是切到了新学期的空课表。

> **必须能退出去**：导入页的每个阶段都要有返回入口（#27）。此前输入页与失败页既无顶栏
> 也无返回回调，从周视图顶栏进来后只能按系统返回键。
...
- 左侧固定节次列（显示节次号 + 起止时间）。
- 顶部日期栏显示日期，当天列高亮。
- **当前时刻高亮（FR-13）**：当前正在上的课加主色边框；已上完的课去饱和变灰。详见 FR-13。
- 支持左右滑动切换周次，顶部显示"第 N 周"。
- 点击课程块进入课程详情。
- 跨多节的课程绘制为一个连续长块。

### FR-05 桌面小组件（P0，#32 重构）

仅提供一个「课表小组件」入口。桌面以四列宽为默认，支持经桌面长按垂直调整为 `4×1`、`4×2`、`4×4`；不同 Launcher 给出中间高度时按最接近的一档渲染。三个尺寸均为相同信息密度的纵向可滚动课程列表。

- 每条 `CourseSession` 独立显示。首行：`起始节次 · 开始时间 课程名`；次行：`结束节次 · 结束时间 地点 · 教师`；右侧固定闹钟图标。
- 每个实例独立选择显示范围：`今日未结束`（默认，保留正在上和之后的课）、`今日全部`、`今日＋明天`、`本周`。后两者按「今天 / 明天 / 周几」分组；范围无课程时显示可点的「下一次：日期 · 课程名」。
- 每个实例独立选择左侧：课程颜色条、`起始节次–结束节次` 条、或两者同时显示。
- App 的「课程管理 → 小组件」进入独立设置页，自动列出已添加实例为「课表小组件 1 / 2 …」，并显示当前尺寸与背景缩略信息。设置写入实例的 `AppWidgetId`，不新增业务数据表字段。
- 背景支持完全透明、纯色、双色渐变、内置预设图、相册图片；纯色和渐变颜色可调。背景与课程卡片共用透明度，文字透明度独立；可选深色预设。
- 点课程、范围空态的下一次课程、或列表空白区，打开课程所在日期的周视图；根节点不得挂点击，保留 Launcher 长按菜单。闹钟入口只进入「课程提醒将在 M8 提供」占位页，不写入提醒数据或申请权限。
- 刷新同时保留写库即时刷新、日期/时区/开机广播、15 分钟 WorkManager 兜底，以及在课程开始/结束时刻的单次精确刷新。
### FR-06 导出备份（P0）
...
- 支持增删节次；所有时间选择使用滚轮，不使用手动文本输入。
- 顶部只设置一组全局单节时长和课间时长；节次仍按第 1 节至第 12 节连续列表展示。
- 点击某节时间时，以滚轮调整该节开始与结束时间。修改结束时间会更新全局单节时长；
  修改某节开始时间会将该节设为断点，断点后的节次按全局规则递推（如下午第一节）。
- 断点可恢复为自动递推；默认时间表的午间、晚间空档会自动识别为断点。
- 保存前校验每节结束时间不得晚于下一节开始时间；冲突或超出当天时间范围时不可保存。
- 全局规则确认成功后，列表立即重算；断点开始时间保持不变。若新规则造成冲突，弹窗在滚轮下方显示原因，保留原列表供继续修改。
- 打开节次直接确认或只改结束时间，不得把自动节次变成断点；只有开始时间真正改变才新增断点（#38）。
- 保存时统一绑定目标学期，校验连续编号和当天时间范围，以事务原子替换；失败保留旧表并提示，成功刷新周视图与小组件（#38）。
- 页面返回键与系统返回键共用离开判断：当前草稿与进入页面时的编辑状态不同且尚未保存时，询问「保存 / 放弃更改 / 继续编辑」；保存成功后才离开，失败留在页面；无效草稿禁用保存但允许放弃或继续编辑（#42）。
- 草稿已恢复进入页面时的状态，或未作更改时，直接返回；「恢复默认」按钮沿用既有的立即保存行为。
- 旧版崩溃已清空节次表时，可在本页重新配置并保存，或使用「恢复默认」重建；已删除的自定义时间无法凭空还原，课程数据不受该恢复操作影响。
- 调试 APK 覆盖安装也必须与设备现有 Room 数据库版本兼容：已有 v2 网课数据时，使用包含 v2 schema 和 1→2 迁移的构建验证节次修复，不得安装 v1 schema 构建造成启动时数据库降级崩溃（#41）。
...
导入预览页（「确认识别结果」）是 AI 输出与落库之间的**最后一道人工关口**，用户必须能在这里
修正 AI 识别不准的**全部**字段，而不是只能改课名。

**交互**：
- 课程卡片**只读**展示：课程名 / 教师 / 颜色圆点 / 时间段列表
- **点某条时间段行 → 弹出底部抽屉**（与周视图抽屉共用 `CourseSectionEditForm`）
- 抽屉两段，作用范围不同：
  - **课程信息**（名称 / 教师 / 颜色）→ 该课**全部**时间段
  - **本节课**（周几 / 起始节 / 结束节 / 周次 / 地点 / 备注）→ **只**当前这一条
- 卡片上保留「拆分为新课」与「删除课程」；抽屉里**不放**删除
- 学期信息卡可改学期名、总周数、**起始日**（起始日选定后对齐到周一）

**周次必须用点选网格，禁止用「起止周」两个输入**：起止表达不了 `1-16单`（单周）与
`1-9,11-18`（跳周）。若用起止，用户打开一门单周课、**什么都不改**直接保存，
`1-16单` 会被写成 `1-16`，这门课从此每周都上 —— 属于**静默数据损坏**。

**边界与兜底（重要）**：
- 周次**为空时必须拦住保存**。空串写进 `weekSpec` 会让 `WeekSpecParser.parseOrNull` 返回 null，
  周视图据此跳过该时间段 → 整门课永久不显示且无任何报错。落库侧统一走
  `core:common/WeekSpecFallback.orFullTerm()`（判 null **与空白**）
- 节次滚轮上限取「默认节次表长度」与「草稿已用到的最大节次」的**较大者**，
  否则 AI 给的更大节次会被夹掉
- `SessionDraft` 字段全为 nullable；AI 漏给周几/节次的记录原本会被静默丢弃，
  现在可在预览页补齐 —— 这是本功能的主要价值之一

**数据契约**：预览页的颜色等编辑只存在于 `MergedCourse`（导入器内部模型），
`confirmImport()` 再映射到已有的 `Course` / `CourseSession` 字段。
**不涉及第 5 章导入格式规范，不需要数据库迁移。**

### FR-13 课块的时间状态（P0）

课块按「相对此刻」分三态，周视图与桌面小组件**共用同一套判定**：

| 状态 | 条件 | 表现 |
|---|---|---|
| `Upcoming` | 结束时刻晚于此刻 | 全彩 |
| `Ongoing` | 此刻处于起止时刻之内 | 全彩 **+ 主色边框** |
| `Past` | 结束时刻早于此刻 | **去饱和变灰** |

**判定只看时间，不看周次** —— 这是有意为之：切成上一周就整周变灰，切到下一周就全彩，
切到更早的学期也全灰。别把它当成 bug 改掉（`BlockPhaseTest` 有对应断言）。

> **实现约束**：判定与灰化必须走 `core:common` 的 `BlockPhaseCalculator` 与 `MutedBlockColor`
> （纯函数、可单测）。周视图与小组件**共用**，不允许各自实现一份。

> **为什么是「去饱和」而不是「半透明」**：半透明只是变淡、颜色还在，看着是「淡」不是「灰」。
> `MutedBlockColor` 把颜色朝自身 Rec.601 亮度收敛（保留少量色相），
> 整体一眼是灰的，但不同课程仍留有明度差异。

**两个必须遵守的边界**：
- **缺起止时间时判 `Upcoming`**（宁可不灰，不要误灰）。节次表没配全时 `ScheduleLayout`
  给不出时间，此时涂灰会让人以为课已经上完了。
- **文字颜色按灰化后的颜色算**。灰化会改变亮度，沿用原色判断可能让浅灰底配白字看不清。

**小组件的额外注意**：小组件是「每格标课名」的缩略形式，一节课跨几节就出现几次，
所以相位**要按这一格所在的节次单独判**，不能整块共用一个结果 ——
否则正在上的那节课会被它后面那节连累成灰。

### FR-15 统一视觉语言（P0，#34）

App 使用统一的「云朵课表」视觉语言：奶油纸张底色、蓝莓主色、蜜桃与薄荷点缀，
配合大圆角卡片、胶囊操作和清晰的标题层级，形成轻松、柔和、偏卡通的整体气质。

- 周视图继续以课程辨识和高信息密度为首要目标；装饰主要放在页头、日期状态和操作控件，
  不减少课程名、时间、地点等既有信息。
- 浅色与深色模式都使用产品自有色板；系统动态取色默认关闭，避免同一 App 在不同壁纸下失去统一风格。
- 课程自定义色仍按用户选择显示，不用主题色覆盖；正在上课、已结束等时间状态规则保持不变。
...
- App 内「提醒管理」提供全局默认提前时间与本学期课程入口；新课程提醒默认关闭，首次开启时采用全局默认时间。
