...
### #30 [需求] 小组件重构为「最近课程列表」：紧凑横条 / 详细 4×4 双入口
...
- 拉伸规则：条数上限由入口决定（紧凑 2 / 详细 4），条目平分剩余空间
  —— 拉伸只改行高/列宽不增减条数
- 点击任意位置 → 打开 App 周视图（沿用 `actionStartActivity`）
- **横条形态是用户中途更正的**：最初按「2 宽 × 4 高竖条」实现，用户指出要的是
  「横着的长条形」—— 2 节并排两列。provider 的 `targetCellWidth/Height` 改为 4×2，
  且 **`minWidth` 必须同步提到 330dp**（Pixel Launcher 按 minWidth 换算默认宽度，
  250dp 只给 3 格）
- **过程中修掉**：根级 `GlanceModifier.clickable`（点按打开 App）会**吞掉 launcher 的
  长按菜单**，导致小组件无法移除 / 调整大小。把 clickable 移到内层内容区后恢复。
  **任何 Glance 小组件都不要在根节点放 clickable**
...
- 技术点：两个 `GlanceAppWidgetReceiver` 子类 + 两份 provider XML，manifest 注册第二项；
  数据加载收敛到 `WidgetLoader`，两个形态共用同一份 `WidgetData`、各自裁条数
- 核心选择逻辑抽到 `core:common/LessonWindow`（纯函数，与 #31 共用）
- 验证：选择器出现两个入口（紧凑 4宽×2高 / 详细 4宽×4高）；
  紧凑横条两节课**并排两列**（B 19:00 / C 20:50，各自带课程色竖条），
  已结束的 A 被过滤；详细 4×4 标题行「9月20日 周日 · 第 1 周」+ 两条竖排；
  切到全上完的学期后显示空态「今天没有更多课」；长按可呼出 Remove/Resize
