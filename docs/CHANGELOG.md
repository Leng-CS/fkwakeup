...
- 导入器支持按明确起止周次推算异步开放日期并标注来源。预览页可补日期、平台和链接、切换直播／异步；缺必需字段或开放期完全位于学期外时禁用确认导入，避免静默丢失。
...
- 分支：`feat/m8-conflict-detection`，commit `67da61a`
- 新增 `CourseConflictDetector` 纯逻辑：同一天、周次集合相交、节次范围重叠时判定冲突；相邻节次不冲突，线下与直播课程统一参与，异步开放期不参与。
- 课程管理编辑、周视图快捷编辑、导入确认三处在写库前执行同一规则。
- 冲突弹窗列出课程、星期、重叠节次与周次；用户可返回修改，也可明确选择「仍然保存/导入」。
- 检测只关注当前准备写入的时间段，不会因课表中原有的其他冲突阻碍一次无关编辑。
- 补 5 个单测，覆盖周次交集、相邻节次、分离周次、线下与直播、同一持久化时间段排除。
- 验证：`:core:common:test :app:assembleDebug` 通过，核心模块共 152 项单测全绿。
